...
func NewAlphabetical(order string, prefix string) (*Alphabetical, error) {
	switch order {
	case "":
...
func (p *Alphabetical) Latest(versions []string) (string, error) {
	...
	var prefixed []string
	if prefix := p.PrefixMatch; prefix != "" {
		for _, ver := range versions {
			if strings.HasPrefix(ver, prefix) {
				prefixed = append(prefixed, ver)
			}
		}
		if len(prefixed) > 0 {
			versions = prefixed
		} else {
			return "", fmt.Errorf("no versions from the list matched prefix '%s'", prefix)
		}
	}
