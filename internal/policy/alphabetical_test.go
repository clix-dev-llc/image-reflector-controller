...
func TestAlphabetical_Latest(t *testing.T) {
	cases := []struct {
		...
		{
			label:           "Ubuntu CalVer with prefix",
			versions:        []string{"16.04", "16.04.1", "16.10", "20.04", "20.10"},
			prefix:          "16",
		...
		{
			label:           "Ubuntu code names with prefix",
			versions:        []string{"dapper", "xenial", "yakkety", "zesty", "artful", "bionic", "disco"},
			prefix:          "d",
			expectedVersion: "disco",
		},
		{
			label:           "Timestamps",
		...
		{
			label:     "No matching prefix versions",
			versions:  []string{"a"},
			prefix:    "z",
			expectErr: true,
		},
	...
	for _, tt := range cases {
		t.Run(tt.label, func(t *testing.T) {
			policy, err := NewAlphabetical(tt.order, tt.prefix)
			if err != nil {
				t.Fatalf("returned unexpected error: %s", err)
			}
